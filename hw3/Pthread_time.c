...
clock_t begin_logic, end_logic;
clock_t logic_time=0;
...
int main(int argc, char** argv){
    ...
    clock_t total_IO = end_IO - begin_IO;
    clock_t total_time = end_total - begin_total;


    printf("IO time = %lf\n", (double)(total_IO) / CLOCKS_PER_SEC);
    printf("logic time = %lf\n", (double)(logic_time/num_threads) / CLOCKS_PER_SEC);
...
void floydWarshall (int *map, int vertice, int *dist, const char* outfile, int num_threads){
    ...
    for (k = 0; k < vertice; k++)
    {
        ...
        end_logic = clock();
        logic_time += (end_logic - begin_logic);
